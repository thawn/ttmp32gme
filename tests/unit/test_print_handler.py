...
class TestCreatePrintLayout:
    ...
    def db(self):
        ...
        # Add test album
        db_handler.write_to_database("gme_library", {
            "oid": 920,
        ...
        # Add test track
        db_handler.write_to_database("tracks", {
            "parent_oid": 920,
            "title": "Test Track",
            "track": 1,
            "duration": 180000,
        ...
        for script, code in [("play", 3947), ("t0", 2663)]:
            try:
                db_handler.write_to_database("script_codes", {
                    "script": script,
                    "code": code
                })
    ...
    @patch('ttmp32gme.print_handler.create_oids')
    @patch('ttmp32gme.print_handler.get_sorted_tracks')
    def test_create_print_layout_basic(self, mock_get_sorted_tracks, mock_create_oids, mock_render_template, db):
...
class TestFormatPrintButton:
    ...
    def test_format_print_button_linux_with_old_wkhtmltopdf(self, mock_run, mock_get_exec, mock_system):
