...
def get_tracks(album: Dict[str, Any], connection) -> Dict[int, Dict[str, Any]]:
    ...
    cursor = connection.cursor()
    cursor.execute(
        "SELECT * FROM tracks WHERE parent_oid=? ORDER BY track", (album["oid"],)
    )
...
def update_table_entry(
    table: str, keyname: str, search_keys: List, data: Dict[str, Any], connection
) -> bool:
...
def create_library_entry(
    album_list: List[Dict], connection, library_path: Path, debug: int = 0
) -> bool:
    ...
    for i, album in enumerate(album_list):
        ...
        track_data.sort(
            key=lambda t: (t.get("disc", 0), t.get("track", 0), t.get("filename", ""))
        )
...
def replace_cover(oid: int, filename: str, file_data: bytes, httpd, connection) -> int:
    ...
    # Update database
    update_table_entry(
        "gme_library", "oid=?", [oid], {"picture_filename": clean_filename}, connection
    )
